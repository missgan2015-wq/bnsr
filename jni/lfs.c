...
{
/* easyLua patch: 编译宏 LFS_USE_LUAJIT_FILE 强制走 5.1 风格 FILE** 取值。
 * 因为我们跑的是 LuaJIT 的 io 库（IOFileUD = FILE* + uint32_t），
 * 与 Lua 5.4 backport 的 LUA_VERSION_NUM=504 不一致，但首字段是 FILE*，
 * 与 5.1 的 FILE** 二进制兼容。 */
#if defined(LFS_USE_LUAJIT_FILE) || LUA_VERSION_NUM == 501
